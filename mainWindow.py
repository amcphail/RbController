...
from moduleEdit import formModuleEdit
...
#        self.tbChannelTab.setWidgetResizable(True)
        ...
        self.pbRemoveModule.setObjectName("pbRemoveModule")
        self.pbEditModule = QtWidgets.QPushButton(self.fButtons)
        self.pbEditModule.setGeometry(QtCore.QRect(10, 10, 80, 20))
        self.pbEditModule.setObjectName("pbEditModule")
        ...
        self.loButtons.addWidget(self.pbEditModule)
...
#        self.fPicture.loadFile('/home/lab/mydata/Programming/RBController/default.fit')
        ...
        self.pbEditModule.clicked.connect(self.editModule)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pbEditModule.setText(_translate("MainWindow", "Edit Module"))
    ...
    def createModule(self):
        createModuleDialog = formModuleSelect(self.num_modules,False,self.num_modules,self.modules,self.MainWindow)
    ...
    def editModule(self):
        editModuleDialog = formModuleEdit(self.num_modules,self.modules,self.MainWindow)
        editModuleDialog.show()

    def modifyModule(self,moduleNumber):
        createModuleDialog = formModuleSelect(moduleNumber,True,self.num_modules,self.modules,self.MainWindow)
        createModuleDialog.show()

    def sanityCheck(self):
        for ii in range(len(self.modules)):
            sane = self.modules[ii].sanityCheck()
            if sane.is_just():
                error = sane.value()
...
#        np.save(sequence.npy,self.sequences)
    ...
    def openFileName(self,fileName):
        ...
        try:
            ...
            error_dialog = QtWidgets.QErrorMessage()
            error_dialog.showMessage('Could not load FITS file')
...
#        self.modules[self.num_modules].show()
    ...
    def updateModule(self,moduleNumber,formModuleSelect):

        index = self.num_modules

        for ii in range(self.num_modules):
            if self.modules[ii].moduleNumber == moduleNumber:
                index = ii

        if index == self.num_modules:
            print("No module: ",index)
            return

        self.modules[index].hide()
        self.gloChannelsContents.removeWidget(self.modules[index])

        # prepare arguments to populateModule
        data = self.modules[index].getEvents()
        # delete old module
        del self.modules[index]
        #   insert new empty channels
        for ii in range(daq.analog_outputs):
            if formModuleSelect.cbAnalog[ii].isChecked():
                if not ii in data[3].keys():
                    data[3][ii] = []
                
        for ii in range(daq.digital_outputs):
            if formModuleSelect.cbDigital[ii].isChecked():
                if not ii in data[4].keys():
                    data[4][ii] = []
                
        for ii in range(daq.network_outputs):
            if formModuleSelect.cbNetwork[ii].isChecked():
                if not ii in data[5].keys():
                    data[5][ii] = []
        # create new module
        self.modules[index] = moduleForm(index,self,self.hlwChannelsContents)
        # populateModule
        self.modules[index].populateModule(*data)

        self.gloChannelsContents.addWidget(self.modules[index])
