...
class ModuleLine(QtWidgets.QWidget):
    def __init__(self, channel, startTime, module, parent = None):
        ...
        self.pbRemoveLineElement.setGeometry(QtCore.QRect(self.left+100,0,30,20))
        ...
        self.pbAddLineElement.setGeometry(QtCore.QRect(self.left+130, 0, 30, 20))
        self.pbAddLineElement.setObjectName("pbAddLineElement")
        self.pbModuleElements[self.num_moduleElements] = self.createButton()
        self.pbModuleElements[self.num_moduleElements].setGeometry(QtCore.QRect(self.left, 0, 100, 20))
    ...
    def sanityCheck(self,start,duration):
        ...
        if self.num_moduleElements == 1:
            event = self.pbModuleElements[0].getEvent()
            if event.is_nothing():
                return Just("No event created yet on channel %d" % self.getChannelNumber())
            else:
                ev = event.value()
                if duration != ev.Duration:
                    return Just("Event does not reach end of module on channel %d" % self.getChannelNumber())
                else:
                    return Nothing()
        else:
            event = self.pbModuleElements[self.num_moduleElements-1].getEvent()
            ev = event.value()
            startTime = ev.startTime
            Duration = ev.Duration
            if (start+duration) != (startTime+Duration):
                return Just("Event does not reach end of module on channel %d" % self.getChannelNumber())
            else:
                return Nothing()
