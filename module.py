...
#        moduleForm.setWindowTitle(_translate("moduleForm", "Form"))
    ...
    def sanityCheck(self):
        ...
        for ii in range(self.num_analogLines):
            sane = self.analogLines[ii].sanityCheck(self.sbStartTime.value(),self.sbDuration.value())
            if sane.is_just():
                return sane

        for ii in range(self.num_digitalLines):
            sane = self.digitalLines[ii].sanityCheck(self.sbStartTime.value(),self.sbDuration.value())
            if sane.is_just():
                return sane
    ...
    def getEvents(self):
        ...
        for ii in range(self.num_analogLines):
            events = self.analogLines[ii].getEvents()
            analog_lines[self.analogLines[ii].getChannelNumber()] = events
        
        for ii in range(self.num_digitalLines):
            events = self.digitalLines[ii].getEvents()
            digital_lines[self.digitalLines[ii].getChannelNumber()] = events
        
        for ii in range(self.num_networkLines):
            events = self.networkLines[ii].getEvents()
            network_lines[self.networkLines[ii].getChannelNumber()] = events
        
        return (name,startTime,Duration,analog_lines,digital_lines,network_lines)
