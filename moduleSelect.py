...
class formModuleSelect(QtWidgets.QDialog):
    ...
    def __init__(self, number, updating, num_modules, modules, parent = None):
        super(formModuleSelect,self).__init__(parent)
 
        self.moduleNumber = number
        ...
        self.bbOkayCancel.accepted.connect(self.addOrUpdateModule)
        ...
        if updating:
            self.setParameters(self.modules[self.moduleNumber].leName.text(),
                               self.modules[self.moduleNumber].sbStartTime.value(),
                               self.modules[self.moduleNumber].sbDuration.value())
            self.populatePreexisting()
    ...
    def setParameters(self,name,start,duration):
    ...
    def updateModule(self):
        self.ui.updateModule(self.moduleNumber,self)
        self.close()

    def addOrUpdateModule(self):
        if self.updating:
            self.updateModule()
        else:
            self.addModule()
